// Augments module: encapsulates augment crafting UI (Runes/Idols/Thesis - refactored to grid + shared image fallback)
...
function deriveTags(e: Augment): string[] {
...
export async function show(): Promise<void> {
  ...
  try {
    ...
    const data = await (api.getAugments ? api.getAugments() : api.getSocketables());
    if (!data || data.error) {
      panel.innerHTML = `<div style='color:var(--accent-red);'>Failed to load Augments (${data?.error||'unknown'})<br><span style='color:var(--text-muted); font-size:11px;'>Run collector scraper</span></div>`;
      return;
    }
    render(data.augments || data.socketables || []);
  } catch (e) {
    panel.innerHTML = `<div style='color:var(--accent-red);'>Exception loading Augments</div>`;
  }
}

export function render(list: Augment[]): void {
  const panel = ensurePanel();
  state.cache = [...(list||[])];
  ...
  panel.innerHTML = `
    <div style='display:flex; gap:6px; align-items:center; margin-bottom:6px;'>
      <input id='augmentSearch' type='text' placeholder='Search augments...' style='flex:1; padding:4px 8px; background:var(--bg-tertiary); border:1px solid var(--border-color); border-radius:4px; color:var(--text-primary); font-size:12px;'>
    ...
    <div style='background:var(--bg-secondary); padding:8px; border-radius:6px; margin-bottom:8px;'>
      <div id='socketableTagFilters' style='display:flex; flex-wrap:wrap; gap:4px; justify-content:center; width:100%;'></div>
    </div>
  ...
  state.input = panel.querySelector('#augmentSearch') as HTMLInputElement | null;
  ...
  function renderTagFilters(){ if(!tagWrap) return; tagWrap.innerHTML='';
    ...
    curated.forEach(tag=>{ const key=tag.toLowerCase(); const active=state.selectedTags.has(key);
      const el=document.createElement('div'); const count=state.tagCounts[tag]||0; el.className='sock-tag'; el.setAttribute('data-tag', key);
      el.textContent = count? `${tag} (${count})` : tag;
      applyFilterChipChrome(el, chipChrome(tag, active), { fontWeight: active ? '600' : '500' });
      el.addEventListener('click',()=>{ if(active) state.selectedTags.delete(key); else state.selectedTags.add(key); apply(state.input?.value||''); renderTagFilters(); });
  ...
  function matchTags(e: Augment){ if(!state.selectedTags.size) return true; const lc=((e as any).__tags||[]).map((t:string)=>t.toLowerCase()); return [...state.selectedTags].every(t=> lc.includes(t)); }
...
export async function reload(): Promise<void> {
  ...
  try {
    const api = (window as any).electronAPI;
    const data = await (api.getAugments ? api.getAugments() : api.getSocketables());
    if (!data || data.error) { panel.innerHTML = `<div style='color:var(--accent-red);'>Failed to load Augments (${data?.error||'unknown'})</div>`; return; }
    render(data.augments || data.socketables || []);
